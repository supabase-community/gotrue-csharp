...
namespace Supabase.Gotrue {
    ...
    public class Client : IGotrueClient<User, Session> {
        ...
        /// <returns></returns>
        public Task<UserList<User>?> ListUsers(string jwt, string? filter = null, string? sortBy = null,
            SortOrder sortOrder = SortOrder.Descending, int? page = null, int? perPage = null) =>
        ...
        /// <param name="attributes"></param>
        /// <returns></returns>
        public Task<User?> CreateUser(string jwt, string email, string password,
        ...
        public async Task<Session?> RefreshSession() {
            ...
            await RefreshToken(CurrentSession.RefreshToken);
        ...
        public async Task<Session?> RetrieveSessionAsync() {
            // If we have a session, and hasn't expired
            if (CurrentSession != null && CurrentSession.ExpiresAt() < DateTime.Now) {
        ...
        public async Task RefreshToken(string? refreshToken) {
            if (CurrentSession == null ||
                string.IsNullOrEmpty(CurrentSession?.RefreshToken) && string.IsNullOrEmpty(refreshToken))
                throw new GotrueException("No current session.", NoSessionFound);

            if (CurrentSession?.ExpiresAt() < DateTime.Now)
                throw new GotrueException("Session is expired.", ExpiredRefreshToken);
